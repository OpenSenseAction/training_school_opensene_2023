...
cml = (xr.open_dataset('../data/cml/openMRG_example.nc') #<----- see 1.1
       .isel(sublink_id = 0, cml_id = 0) #<----------------------- see 1.2
       .sel(time=slice('2015-08-27','2015-08-28')).load()) #<----- see 1.3 and 1.4
cml['tsl'] = cml.tsl.where(cml.tsl != 255.0) #<------------------- see 2.2 (not needed as openMRG uses floats)
cml['rsl'] = cml.rsl.where(cml.rsl != -99.9) #<------------------- see 2.2 (not needed as openMRG uses floats)
